...
class LLMRequest(BaseModel):
    ...
    sanitization_mode: SanitizationMode = Field(
        default=SanitizationMode.BALANCED, description="Sanitisation mode for detected threats"
    )
...
async def proxy_llm_request(request: Request, body: LLMRequest) -> LLMProxyResult:
    ...
    try:
        ...
        result = LLMProxyResult(
            ...
            sanitized_prompt=(
                sanitized_prompt[:500] if len(sanitized_prompt) > 500 else sanitized_prompt
            ),
