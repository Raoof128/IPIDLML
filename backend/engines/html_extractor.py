...
class HTMLExtractor:
    ...
    def extract(
        ...
        try:
            ...
            result = self._extract_with_beautifulsoup(
                html_content, extract_alt_text, detect_hidden
            )
        except ImportError:
            ...
            for pattern in self.hidden_patterns:
                if re.search(pattern, html_content, re.IGNORECASE):
                    ...
                    result["hidden_content"].append({
                        "pattern": pattern,
                        "type": "style_based_hiding"
                    })
        
        # Check for suspicious scripts
        script_matches = re.findall(r'<script[^>]*>(.*?)</script>', html_content, flags=re.DOTALL | re.IGNORECASE)
        ...
        for pattern in self.injection_indicators:
            ...
            for match in matches:
                found.append({
                    ...
                    "severity": "high" if "override" in pattern or "jailbreak" in pattern else "medium"
                })
